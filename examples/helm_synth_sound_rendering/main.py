...
from helm_utils import make_helm_json_parameter_mapping, load_helm_preset 
...
class Helm_Worker:
    ...
    def process_item(self, item: Item):
        ...
        json_mapping = make_helm_json_parameter_mapping(self.synth, preset_path, os.path.join(os.path.dirname(__file__), 'helm_parameter_mappings'))
        self.synth = load_helm_preset(self.synth, json_mapping)
        bname = basename(preset_path).split('.')[0]
        ...
        for pitch in range(self.pitch_low, self.pitch_high+1):
            ...
            sf.write(str(output_path), audio.transpose(), self.sample_rate)
...
def main(plugin_path, preset_dir, note_duration=2, render_duration=4,
    ...
    preset_paths = list(glob(str(Path(preset_dir) / '*.helm'))) # the .pjunoxl is formatted just like an xml file
    ...
    num_processes = int(num_workers) if num_workers else multiprocessing.cpu_count()
    ...
    with multiprocessing.Pool(processes=num_processes) as pool:
        ...
        for i in range(num_processes):
            worker = Helm_Worker(input_queue, plugin_path,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output-dir', default=os.path.join(os.path.dirname(__file__),'output-helm'), help="Output directory.")
